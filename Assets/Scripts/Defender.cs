...
public class Denfender : MonoBehaviour
{
    private float health;
    private float spiritual;
    private int cost;
    private int level;
    private float totalHealth;
    private float totalSpiritual;
    ...
    protected virtual IEnumerator Attack()
    {
        while (m_targetEnemy == null || !m_isFaceEnemy) // 如果没有目标一直等待
            yield return 0;
        m_ani.CrossFade("attack", 0.1f); // 播放攻击动画，暂为“attack”

        while (!m_ani.GetCurrentAnimatorStateInfo(0).IsName("attack")) // 等待进入攻击动画，暂为“attack”
            yield return 0;
        ...
        m_ani.CrossFade("idle", 0.1f); // 播放待机动画，暂为“idle”
